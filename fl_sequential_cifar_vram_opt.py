"""
Federated Averaging (FedAvg) - Sequential Training  [VRAM-optimised variant]
Multiple clients, non-IID CIFAR-10 data, one GPU, Model: ResNet-18.

Key difference vs fl_sequential_cifar.py
-----------------------------------------
After each client finishes training we:
  1. deepcopy the state_dict (weights stay on CPU as plain tensors)
  2. call model.cpu()            → moves all parameters/buffers off VRAM
  3. call torch.cuda.empty_cache() → releases the freed VRAM back to the OS/
                                     CUDA memory pool immediately

This means only ONE client model lives on the GPU at a time, so peak VRAM
scales with (global_model + 1 local model) instead of
(global_model + NUM_CLIENTS local models).  For 100+ clients or larger
models this is the difference between fitting in 8 GB and running OOM.

FedAvg aggregation is also done on CPU to keep GPU pressure low.
"""
...
def train_client(model, loader, device, epochs=2, lr=0.01, client_id=None):
    ...
    Returns a CPU state_dict, then moves model back to CPU and clears VRAM.
    """
    ...
    # ── VRAM optimisation: snapshot weights on CPU, then free GPU memory ──────
    weights = copy.deepcopy(model.state_dict())   # tensors copied to CPU
    model.cpu()                                    # parameters/buffers off GPU
    torch.cuda.empty_cache()                       # return freed blocks to pool
    return weights                                 # caller gets CPU state_dict
...
def main():
    ...
    print(f"Clients: {NUM_CLIENTS}  |  Rounds: {NUM_ROUNDS}  |  Dataset: CIFAR-10  |  "
          f"Model: ResNet-18  |  Mode: SEQUENTIAL (VRAM-optimised)\n")
    ...
    print(f"\n{'='*60}")
    print("SEQUENTIAL SUMMARY (CIFAR-10 / ResNet-18 / VRAM-optimised)")
    print(f"{'='*60}")
    ...
    print()
    print(f"  {'Round':<8} {'VRAM all 5 offloaded (MB)':<28} {'Avg peak/client (MB)':<24} {'Test Acc (%)':<12}")
    print(f"  {'-'*8} {'-'*28} {'-'*24} {'-'*12}")
    for s in round_stats:
        print(f"  {s['round']:<8} {s['vram_after_offload']:<28.0f} {s['avg_peak']:<24.0f} {s['acc']:<12.2f}")
    print()
