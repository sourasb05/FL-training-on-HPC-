"""
FedAvg - Multi-GPU + VRAM-optimised  (N clients, M GPUs, N >= M)
=================================================================
Architecture:
  ...
  - M GPU worker processes: each owns one GPU, trains clients assigned to it
                            sequentially with VRAM offload between clients

Key difference vs fl_fedavg_multigpu.py
----------------------------------------
  - N clients are distributed round-robin across M GPUs (N can exceed M)
  - Each GPU worker trains its assigned clients ONE AT A TIME (sequential)
  - After each client finishes: weights copied to CPU, model.cpu() +
    empty_cache() frees VRAM immediately — only 1 client model per GPU at a time
  - Global model lives on CPU; moved to GPU only during training and eval
...
# ── Train one client on a GPU, offload immediately after ──────────────────────
def train_one_client(global_state_bytes, client_indices, device,
                     batch_size, local_epochs, data_path, client_id):
    """
    Loads global weights onto GPU, trains, copies weights to CPU,
    then offloads model from GPU before returning.
    ...
    # Load global weights and move to GPU
    ...
    # ── VRAM offload: snapshot weights on CPU, free GPU immediately ───────────
    cpu_weights = copy.deepcopy(model.state_dict())  # CPU tensors
    model.cpu()                                        # move model off GPU
    torch.cuda.empty_cache()                           # release VRAM pool
...
def gpu_worker(gpu_id, assigned_clients, num_rounds, local_epochs,
    ...
    One process per GPU. Each round:
      ...
      2. Trains each assigned client SEQUENTIALLY with VRAM offload
...
def server(num_clients, num_gpus, num_rounds, local_epochs,
    ...
    # Assign clients round-robin to GPUs
    ...
    for rnd in range(1, num_rounds + 1):
        ...
        # Broadcast global model bytes to all GPU workers
        ...
        # Evaluate: move to GPU 0, evaluate, move back to CPU
    ...
    print(f"\n{'='*60}")
    print(f"SUMMARY (CIFAR-10 / ResNet-18 / {num_clients} clients / {num_gpus} GPUs / VRAM-opt)")
    print(f"{'='*60}")
    ...
    print("  VRAM offload after each client: only 1 model per GPU at a time.")
...
# ── Entry point ───────────────────────────────────────────────────────────────
def main():
    NUM_CLIENTS  = 10   # can exceed num_gpus
    NUM_ROUNDS   = 5
    LOCAL_EPOCHS = 5
    BATCH_SIZE   = 128
    DATA_PATH    = "./data"

    # Round boundary log — same job ID as nvidia-smi CSV for easy alignment
    ...
    print(f"Clients        : {NUM_CLIENTS}  ({NUM_CLIENTS // num_gpus}–"
          f"{(NUM_CLIENTS + num_gpus - 1) // num_gpus} per GPU, sequential + VRAM offload)")
    print(f"Rounds         : {NUM_ROUNDS}")
    print(f"Local epochs   : {LOCAL_EPOCHS} per round")
    print(f"Dataset        : CIFAR-10  |  Model: ResNet-18")
    print(f"Mode           : Multi-GPU + VRAM-optimised\n")
